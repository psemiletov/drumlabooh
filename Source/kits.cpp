...
void CDrumKit::load_sfz (const std::string &data)
{
  ...
  // change crlf in data to lf
  std::string temp_data = string_replace_all (data, "\r\n", "\n");
  temp_data = string_replace_all (temp_data, "\\", "/");  // ФИКС: используем temp_data, а не data
  ...
  // Helper function to check if line contains unsupported features
  auto has_unsupported_features = [](const std::string& line) -> bool {
      // Horizontal layers (round-robin, random)
      if (line.find("lorand") != std::string::npos ||
          line.find("hirand") != std::string::npos ||
          line.find("seq_position") != std::string::npos ||
          line.find("seq_length") != std::string::npos)
          return true;
      
      // Other unsupported features that could cause issues
      if (line.find("sample=") == std::string::npos)  // No sample in this line
          return false;
  ...
  auto commit_region = [&]()
       {
        ...
        // Skip if no sample file specified
        if (fname.empty())
           {
            region_scope = false;
            return;
        ...
        // VALIDATION 1: Check if key is valid (0-127)
        if (key < 0 || key > 127)
           {
            std::cerr << "CDrumKit::load_sfz: Skipping region with invalid key=" << key << std::endl;
            region_scope = false;
            fname.clear();
            just_name.clear();
            key = -1;
        ...
        // VALIDATION 2: Check if this region uses unsupported features
        // This is already checked before calling commit_region, but double-check
        if (! fname.empty() && has_unsupported_features(line))
           {
            std::cerr << "CDrumKit::load_sfz: Skipping region with unsupported features (round-robin/random) for sample: " << fname << std::endl;
            region_scope = false;
            fname.clear();
            just_name.clear();
            key = -1;
           ...
           temp_sample = map_samples[key];  // Get existing sample
        else
            {
             if (sample_counter >= MAX_SAMPLES)
                {
                 std::cerr << "CDrumKit::load_sfz: Maximum samples reached (" << MAX_SAMPLES << "), skipping region" << std::endl;
                 region_scope = false;
                 fname.clear();
                 key = -1;
                 return;
        ...
        else if (temp_sample && !file_exists(fname))
           {
            std::cerr << "CDrumKit::load_sfz: Sample file not found: " << fname << std::endl;
           }
  ...
  for (size_t i = 0; i < t_str.size(); i++)
      {
       ...
       // Check if line contains unsupported features - skip entire region if found
       if (has_unsupported_features(line))
          {
           // If we're in a region, abort it
           if (region_scope)
              {
               region_scope = false;
               fname.clear();
       ...
       if (line.find ("<region>") != std::string::npos)
          {
           commit_region();
           region_scope = true;
           // Don't parse parameters from the <region> tag itself
           continue;
          }
       ...
       if (line.find ("<group>") != std::string::npos)
          {
           commit_region();
           region_scope = true;
           // Parse group parameters but don't reset key completely
           // Group parameters will be used as defaults for following regions
           continue;
          }
          
       if (line.find ("<control>") != std::string::npos)
          {
           // Just skip control section, nothing to do
           continue;
          }
       ...
       std::string temp_file_just_name = sfz_extract_sample_filename (line);
       if (! temp_file_just_name.empty())
          {
           just_name = rtrim (temp_file_just_name);
           // Build full path: kit_dir + default_path + filename
           if (!sfz_default_path.empty())
              fname = kit_dir + "/" + sfz_default_path + just_name;
           else
       ...
       if (! lovel.empty())
          {
           umin = std::stoi(lovel);
           // Ensure umin is within valid range
           if (umin < 0) umin = 0;
           if (umin > 127) umin = 127;
          }
       ...
       if (! hivel.empty())
          {
           umax = std::stoi(hivel);
           // Ensure umax is within valid range
           if (umax < 0) umax = 0;
           if (umax > 127) umax = 127;
          }
          
       // Check if next line is end of region (group, region, or EOF)
       bool is_last_region = false;
       if (i + 1 == t_str.size())
          is_last_region = true;
       else
          {
           std::string next_line = t_str[i + 1];
           if (next_line.find ("<group>") != std::string::npos ||
               next_line.find ("<region>") != std::string::npos ||
               next_line.find ("<control>") != std::string::npos)
              is_last_region = true;
          }
          
       if (is_last_region && region_scope)
          {
           commit_region();
          }
      }

  // After the loop, commit any remaining region
  commit_region();                                                                 
 
  // Finalize: set sample names from first layer's filename
  for (size_t i = 0; i < MAX_SAMPLES; i++)
      {
       temp_sample = a_samples[i];
         
       if (temp_sample && temp_sample->v_layers.size() > 0)
           {
            temp_sample->name = guess_sample_name (temp_sample->v_layers[0]->file_name);
            
            // Debug output
            // std::cout << "Loaded sample: key=" << temp_sample->mapped_note 
            //          << ", name=" << temp_sample->name 
            //          << ", layers=" << temp_sample->v_layers.size() << std::endl;
           }
      }
